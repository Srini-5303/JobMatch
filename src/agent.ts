...
//             console.warn("API rate limit exceeded. Using fallback parser.");
//           }
//         } else if (apiError.status === 401) {
...
//           console.warn(`API error (${apiError.status}). Using fallback parser.`);
//         }
//       } else if (err instanceof Error) {
...
//         console.warn("  JSON parsing error, using fallback:", err);
...
//     } else {
//       console.warn(" Resume analysis API call completed but no text was received");
//     }
...
//         } else {
//           console.warn(`  Cover letter - API error (${apiError.status})`);
//         }
//       } else if (err instanceof Error) {
...
//     } else {
//       console.warn("  Cover letter API call completed but no text was received");
//     }
...
//       console.log("  Warning: Cover letter seems short. First 150 chars:", coverLetter.substring(0, 150));
//     }
...
//       console.warn("  Warning: Cover letter response may contain prompt/instruction text. Cleanup applied.");
//     }
    
//     // Fallback if empty or too short
//     if (!coverLetter || coverLetter.length < 100) {
//       console.warn("  Cover letter too short or empty, using fallback template");
...
// src/agent.ts
// Thin orchestrator — re-exports everything so server.ts needs no changes

export { analyzeJDResume } from "./analyzers/jd_resume.ts";
export { analyzeResumeStrength } from "./analyzers/resume.ts";
export { generateCoverLetter } from "./analyzers/cover_letter.ts";
export { searchAndRankJobs } from "./jobs/search.ts";
export type { JobSearchPreferences, ResumeStrengthAnalysis, JobSearchResult } from "./types.ts";
