...
const NON_TECHNICAL_TERMS = [
  "competitive salaries", "benefits", "benefi   ts package", "salary", "compensation",
...
async function runAgent(prompt: string): Promise<string> {
  // createAgent() uses the cached context from providers.ts
  // which sets HOME from USERPROFILE before Zypher runs
  ...
  try {
    ...
    else console.warn("⚠️  API error. Using fallback.", err);
    return "";
  }
  return resultText;
}
